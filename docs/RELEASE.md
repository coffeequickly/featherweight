...
- 2.5: "Fix: superscript/subscript text and bulleted/numbered lists stay exactly as Figma renders them (as outlines, with the reason in the report). Inter is now the 3.19 build Figma bundles, so Inter text matches Figma's widths and weight, and its Semi Bold/Extra Bold/Extra Light are found in the catalog. Text baselines sit exactly where Figma puts them. Masked, blended, clipped and translucent-group text stays as outlines. The Fonts screen shows which build of a font is embedded; text set in a different build than Figma rendered stays as outlines and says so. Fit to Size counts an image used on many pages correctly and says when the target is missed. Cancel stops at once and saves nothing. A font-subset error no longer hangs the export; transparent PNGs are never turned into JPEG."
