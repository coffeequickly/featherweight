...
> **2026-09-09 주:** 아래는 최초 스펙이다. 화면은 그 뒤로 두 번 크게 바뀌었다.
> 2.0 에서 탭을 걷고 한 화면 + 체크리스트로 갔다가, **3.0 에서 다시 탭으로 돌아왔다** —
> 이번에는 기능이 아니라 작업 순서로 나눈 여섯 개(시작·정렬·폰트·이미지·옵션·결과)다.
> 그 근거와 화면별 정의는 [UI 리디자인 작업정의서](UI-REDESIGN-SPEC.md)에 있고,
> 문구 기준은 [UI 문구·번역 작성 기준](UI-COPY-GUIDE.md)에 있다.
> 현재 화면 기준은 README 와 `npm run ui:preview` 실물이다.

- `@create-figma-plugin/ui` 컴포넌트만 사용. **3.0 부터 다크 고정** — `--figma-color-*` 를
  `!important` 로 덮는다(사용자 테마를 따라가지 않는다). 폭 440, 높이 560, resizable.
