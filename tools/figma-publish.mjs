// Figma Community 에 새 버전을 올린다 — 코드만 갱신하는 용도. **수동 실행 전용.**
//
//   FIGMA_WEB_AUTHN_TOKEN=... node tools/figma-publish.mjs "릴리즈 노트"
//   node tools/figma-publish.mjs --dry-run          현재 스토어 상태만 확인
//
// CI 에는 일부러 넣지 않았다. 인증이 만료되는 쿠키인데 자동 갱신·로테이션 수단이 없어서,
// 자동화하면 "평소엔 되다가 어느 날 조용히 실패하는" 릴리즈 경로가 된다. 계정
// 자격증명급 시크릿을 CI 에 상주시키는 위험도 함께 진다. 평소에는 데스크톱 앱에서
// Plugins → Manage plugins → Publish new version (클릭 두 번)이 더 낫다.
// 이 스크립트는 그 클릭이 곤란할 때(원격·반복 배포)나 공식 API 가 생겼을 때의 출발점이다.
//
